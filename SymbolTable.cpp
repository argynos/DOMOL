...
string SymbolTable::mostrar() {
    ...
    for (int i = 0; i < this->size(); i++) {
        ...
        cad = cad + "\t\t";        
        if(this->tablaSimbolos[i]->tipo == 0 ) {
            cad += "entero";
            cad = cad + "\t\t";
            cad += static_cast<std::ostringstream*>(&(std::ostringstream() << this->tablaSimbolos[i]->valor.valor_entero))->str();
        } else if(this->tablaSimbolos[i]->tipo == 1 ) {
            cad += "real";            
            cad = cad + "\t\t";
            cad += static_cast<std::ostringstream*>(&(std::ostringstream() << std::setprecision(2) << this->tablaSimbolos[i]->valor.valor_real))->str();
        } else if(this->tablaSimbolos[i]->tipo == 2 ) {
            cad += "lógico";
            cad = cad + "\t\t";
